...
public class SqlUtils {
	...
	public void checkOrCreateTable(Class<?> clazz) {
		String sql = "CREATE TABLE IF NOT EXISTS " + SQLConstants.TABLE_PREFIX + StrUtil.toUnderlineCase(clazz.getSimpleName()) + SQLConstants.TABLE_SUFFIX + " (id VARCHAR(32) NOT NULL PRIMARY KEY)";
	...
	public void checkOrCreateIndex(Class<?> clazz, String[] colums, boolean unique, List<Map<String, Object>> indexs) {
		...
		if (!hasIndex) {
			...
			for (String colum : colums) {
				columList.add(StrUtil.toUnderlineCase(SQLConstants.COLUMN_PREFIX + colum + SQLConstants.COLUMN_SUFFIX + length));
			}

			String sql = "CREATE " + type + "  " + SQLConstants.TABLE_PREFIX + StrUtil.toUnderlineCase(name) + SQLConstants.TABLE_SUFFIX + " ON " + SQLConstants.TABLE_PREFIX + StrUtil.toUnderlineCase(clazz.getSimpleName()) + SQLConstants.TABLE_SUFFIX + "(" + StrUtil.join(",", columList) + ")";
			logQuery(formatSql(sql));
			jdbcTemplate.execute(formatSql(sql));
		}
	...
	public void checkOrCreateColumn(Class<?> clazz, String name, Set<String> columns) {
		if (!columns.contains(StrUtil.toUnderlineCase(name).toLowerCase())) {
			String sql = "ALTER TABLE " + SQLConstants.TABLE_PREFIX + StrUtil.toUnderlineCase(clazz.getSimpleName()) + SQLConstants.TABLE_SUFFIX + " ADD COLUMN " + SQLConstants.COLUMN_PREFIX + StrUtil.toUnderlineCase(name) + SQLConstants.COLUMN_SUFFIX + " TEXT";
			logQuery(formatSql(sql));
	...
	public void updateDefaultValue(Class<?> clazz, String column, String value) {
		String sql = "SELECT COUNT(*) FROM " + SQLConstants.TABLE_PREFIX + StrUtil.toUnderlineCase(clazz.getSimpleName()) + SQLConstants.TABLE_SUFFIX + " WHERE " + SQLConstants.COLUMN_PREFIX + StrUtil.toUnderlineCase(column) + SQLConstants.COLUMN_SUFFIX + " IS NULL";
		logQuery(formatSql(sql));
		...
		if (count != null && count > 0) {
			sql = "UPDATE " + SQLConstants.TABLE_PREFIX + StrUtil.toUnderlineCase(clazz.getSimpleName()) + SQLConstants.TABLE_SUFFIX + " SET " + SQLConstants.COLUMN_PREFIX + StrUtil.toUnderlineCase(column) + SQLConstants.COLUMN_SUFFIX + " = ? WHERE " + SQLConstants.COLUMN_PREFIX + StrUtil.toUnderlineCase(column) + SQLConstants.COLUMN_SUFFIX + " IS NULL";
			logQuery(formatSql(sql));
